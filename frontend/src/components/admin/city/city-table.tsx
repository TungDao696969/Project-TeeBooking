...
interface CityTableProps<TData> {
  columns: ColumnDef<TData, unknown>[];
  data: TData[];
}

export default function CityTable<TData>({
  columns,
  data,
}: CityTableProps<TData>) {
      ...
      <div className="flex items-center justify-between border-b border-[#2a2a2a] px-5 py-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 rounded-lg border border-[#3a3a3a] bg-[#2a2a2a] px-3 py-2">
            ...
            <span className="text-sm text-[#666]">Tìm kiếm thành phố...</span>
          </div>
          <Link href={"/admin/city/create"}>
            <Button className="bg-[#c8000a] text-white hover:bg-[#a00008]">
              <Plus className="mr-2 h-4 w-4" /> Thêm thành phố
            </Button>
          </Link>
        </div>
      </div>
