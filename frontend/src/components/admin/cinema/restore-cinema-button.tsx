...
import { RotateCcw, CircleCheck, Building2 } from "lucide-react";
import { useRestoreCinema } from "@/hooks/admin/cinema/use-restore-cinema";

interface Props {
  cinemaId: string;
}

export default function RestoreCinemaButton({ cinemaId }: Props) {
  const { mutate, isPending } = useRestoreCinema();

  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <button className="inline-flex h-9 w-9 items-center justify-center rounded border border-[#2a2a2a] bg-transparent text-[#777] transition-colors hover:border-[rgba(200,16,46,0.4)] hover:bg-[rgba(200,16,46,0.06)] hover:text-[#c8102e]">
        ...
        <div className="px-7 pt-7 pb-5">
          <AlertDialogHeader className="text-left">
            ...
            <AlertDialogDescription className="mt-1.5 text-[13.5px] leading-relaxed text-[#666]">
              Rạp sẽ được đưa trở lại danh sách hoạt động và có thể hiển thị
              lại.
            </AlertDialogDescription>
            ...
            <div className="mt-4 flex items-stretch overflow-hidden rounded">
              ...
              <div className="flex flex-1 items-center justify-center gap-1.5 border-y border-[rgba(39,174,96,0.15)] bg-[rgba(39,174,96,0.07)] px-3 py-2">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[1px] text-[#27ae60]">
                  Dữ liệu rạp sẽ được giữ nguyên
                </span>
              </div>
              <div className="w-3.5 shrink-0 border border-[#222] bg-[#0f0f0f]" />
            </div>
