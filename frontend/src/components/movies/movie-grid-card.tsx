...
export default function MovieGridCard({ movie }: Props) {
  ...
  const renderGenres = () => {
    if (!movie.genres) return "Kinh Dị";
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-[#0B172E] flex flex-col h-full border border-white/5 shadow-md">
      ...
      <div className="relative overflow-hidden rounded-t-2xl aspect-[2/3] w-full">
        ...
        {/* Overlay */}
        <div className="absolute inset-0 bg-black/80 opacity-0 transition-all duration-500 group-hover:opacity-100" />

        {/* Content Hover */}
        <div className="absolute inset-0 flex flex-col justify-center px-6 text-white opacity-0 translate-y-6 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0 z-10">
