...
const sendBookingConfirmationEmail = async (bookingId) => {
    try {
        ...
        if (!booking || !booking.user?.email) {
            console.warn(`[BookingEmail] Booking ${bookingId} not found or has no email — skipping.`);
            return;
        }
        // ── 2. Build QR buffers for each ticket (CID approach) ────────────────
        const attachments = [];
        const tickets = await Promise.all(booking.tickets.map(async (ticket, index) => {
        ...
        const showtimeStart = booking.showtime?.startTime
            ? (0, dayjs_1.default)(booking.showtime.startTime)
            : null;
        ...
        const result = await (0, mail_1.sendMailTemplateWithAttachments)(booking.user.email, `🎬 Xác nhận đặt vé - ${booking.bookingCode}`, "booking-confirmation", context, attachments);
        if (result) {
            console.log(`[BookingEmail] ✅ Confirmation email sent to ${booking.user.email} for booking ${booking.bookingCode}`);
        }
        else {
            console.error(`[BookingEmail] ❌ Failed to send confirmation email for booking ${booking.bookingCode}`);
        }
    }
    catch (err) {
        // Non-blocking: log error but don't crash the payment flow
        console.error(`[BookingEmail] ❌ Unexpected error for booking ${bookingId}:`, err);
    }
};
exports.sendBookingConfirmationEmail = sendBookingConfirmationEmail;
//# sourceMappingURL=send-booking-email.service.js.map